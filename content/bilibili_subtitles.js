...
(function() {
  // ========== 同步阶段（在 MAIN 世界脚本之前执行） ==========
  // 由于 manifest 中本脚本排在 MAIN 世界脚本之前，以下代码先于 bilibili_1080p_inject.js 运行

  const inIncognito = chrome.extension.inIncognitoContext;

  // 1. 注入 __aio_incognito 全局变量到 MAIN 世界
  try {
    const flagScript = document.createElement('script');
    flagScript.textContent = `window.__aio_incognito=${inIncognito};`;
    (document.head || document.documentElement).appendChild(flagScript);
    flagScript.remove();
  } catch(e) {}

  // 2. 在无痕模式下，同步检查缓存标记并提前写入 localStorage
  //    这确保即使 async storage 回调还没返回，localStorage 也已包含正确的值
  if (inIncognito) {
    try {
      const cachedPND = localStorage.getItem('aio_pnd_cache');
      if (cachedPND === '1') {
        // 强制写入播放器配置
  ...
  function loadSettings() {
    ...
    chrome.storage.sync.get([
      ...
      // 3. 更新播完暂停的本地缓存标记（供下次同步阶段使用）
      if (inIncognito) {
        localStorage.setItem('aio_pnd_cache', res.biliPlayNextDisabled ? '1' : '0');
        // 立即应用（此时 MAIN 脚本已运行但 mainLoop 会持续同步）
        if (res.biliPlayNextDisabled) {
          try {
            const raw = localStorage.getItem('bpx_player_profile');
            const profile = JSON.parse(raw || '{}');
            if (!profile.play) profile.play = {};
